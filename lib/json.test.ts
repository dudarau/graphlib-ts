import { Graph, GraphLabel } from './graph';
...
describe('json', function () {
  ...
  it('preserves the graph value, if any', function () {
    expect(rw(new Graph().setGraph(1 as any)).graph()).equals(1);
    expect(rw(new Graph().setGraph({ foo: 'bar' } as GraphLabel)).graph()).eqls({ foo: 'bar' });
